...
export const getAllCompaniesAdmin = async (req, res) => {
  try {
    ...
    const [companies, total] = await Promise.all([
      Company.find(filter).populate("user", "fullName email").skip(skip).limit(Number(limit)).sort({ createdAt: -1 }),
...
export const getCompanyByIdAdmin = async (req, res) => {
  try {
    ...
    const company = await Company.findById(id).populate("user", "fullName email");
...
export const createCompany = async (req, res) => {
  try {
    const { user, headline = "", description = "", website = "", location = "", size = "" } = req.body;

    if (!user) {
      return res.status(400).json({ success: false, message: "Trường 'user' là bắt buộc" });
    }
    ...
    if (existing) {
      return res.status(400).json({ success: false, message: "User này đã có company" });
    }

    const company = new Company({ user, headline, description, website, location, size });
...
export const updateCompany = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Cập nhật company thành công", data: updated });
  } catch (error) {
